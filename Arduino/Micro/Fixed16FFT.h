...
typedef struct
{
  int16_t frequencies[5];
  int16_t amplitudes[5];
} five_frequencies;
...
class Fixed16FFT {
  ...
  public:
    ...
    five_frequencies five_max_frequencies(fixed_t x[], const int size, float frequency);
...
uint8_t Fixed16FFT::fft(fixed_t x[], const int size) {
  if(size == 1)
    return 0;

  /* indices */
  uint16_t i,j,k,n_1;
  uint8_t array_num_bits;
  uint16_t n_2 = 1;
...
*/
uint8_t Fixed16FFT::bit_reverse(const uint8_t nbits, uint8_t val) {
  switch(nbits) {
    case 8:
...
fixed_t Fixed16FFT::fixed_mul(fixed_t a, fixed_t b) {
  fixed_t result;
  asm (
...
fixed_t Fixed16FFT::fixed_add_saturate(fixed_t a, fixed_t b) {
  fixed_t result;
  asm (
      "movw %A[result], %A[a]" "\n\t"
      "add %A[result],%A[b]" "\n\t" 
...
/* Approximate modulus with a 5% margin error. 
   ...
   for why it works.
*/
float Fixed16FFT::modulus(fixed_t x[], const int size, float frequency) {
  uint16_t i, i_maxi;
  fixed_t a,b;
  fixed_t maxi=0;
  for(i=0; i<size/2; i++) {
...
five_frequencies Fixed16FFT::five_max_frequencies(fixed_t x[], const int size, float frequency) {
  uint16_t i, j, k, temp_k;
  uint16_t i_maxi[5] = {0, 1, 2, 3, 4};
  fixed_t maxi[5]= {0, 0, 0, 0, 0};
  fixed_t a, b, y, temp_y;
  for(i=0; i<size/2; i++) {
    a = abs(x[2*i]);
    b = abs(x[2*i+1]);
    x[i] = max(fixed_mul(ONE_OVER_SQRT_TWO, fixed_add_saturate(a, b)), max(a, b));
  ...
  five_frequencies result;

  for(j = 0; j < 5; j++) {
      result.frequencies[j] = i_maxi[j] * frequency/size;
      result.amplitudes[j] = maxi[j];
  }
